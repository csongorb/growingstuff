window.data = [
  {
    title: "The most basic tap simulator",
    date: "2025-01-09 21:48",
    desc: "Starting point, early sketching.",
    changes: [
      "Starting point, early sketching."
    ],
    img: "250109_2148.png",
    folder: "250109_2148"
  },
  {
    title: "Swipe prototyping",
    date: "2025-01-16 20:39",
    desc: "Random left or right swipes, animation.",
    changes: [
      "Random left or right swipes",
      "Animation"
    ],
    img: "250116_2039.png",
    folder: "250116_2039"
  },
  {
    title: "Acts and basic keyboard",
    date: "2025-01-23 20:13",
    desc: "Added gong, instructions, some refactoring, added keyboard.",
    changes: [
      "Added gong",
      "Added instructions",
      "Some refactoring", 
      "Added keyboard"
    ],
    img: "250123_2013.png",
    folder: "250123_2013"
  },
  {
    title: "Basic 'Phoning' interaction",
    date: "2025-02-17 19:30",
    desc: "Upgraded to OOP, refined interactions and animations (drag still broken?), more acts.",
    changes: [
      "Upgraded to OOP", 
      "Refined interactions and animations (drag still broken?)", 
      "More acts"
    ],
    img: "250215_0006.png",
    folder: "250217_1930"
  },
  {
    title: "Better drag and tutorial",
    date: "2025-02-20 20:51",
    desc: "Bug fixes, instructions.",
    changes: [
      "Bug fixes", 
      "Instructions"
    ],
    img: "250220_2051.png",
    folder: "250220_2051"
  },
  {
    title: "Basic menu with standard and zen",
    date: "2025-02-24 21:04",
    desc: "Basic sound, fading instructions.",
    changes: [
      "Basic sound", 
      "Fading instructions"
    ],
    img: "250224_2104.png",
    folder: "250224_2104"
  },
  {
    title: "Bye zen",
    date: "2025-02-26 21:14",
    desc: "No sound for standard, new texts, placeholder info screen, no zen mode.",
    changes: [
      "No sound for standard", 
      "New texts", 
      "Placeholder info screen",
      "Removed zen mode"
    ],
    img: "250226_2114.png",
    folder: "250226_2114"
  },
  {
    title: "Kind of done (release)?",
    date: "2025-03-05 15:33",
    desc: "Menu improvements, tweaking UI, worked on acting, portrait perfected.",
    changes: [
      "Menu improvements", 
      "Tweaking UI", 
      "Worked on acting", 
      "Portrait perfected"
    ],
    img: "250303_2041.png",
    folder: "250305_1533"
  }
];
